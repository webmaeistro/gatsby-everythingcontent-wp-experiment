const shadowKeyUmbraOpacity = 'rgba( 0, 0, 0, 0.2 )'
const shadowKeyPenumbraOpacity = 'rgba( 0, 0, 0, 0.14 )'
const shadowKeyAmbientOpacity = 'rgba( 0, 0, 0, 0.12 )'
...
export const elevation = (level, more = false) => {
  return `${shadowKeyUmbra[level]},
            ${shadowKeyPenumbra[level]},
            ${shadowKeyAmbient[level]}${more ? ',' : ';'}`
}

export const elevationTransition = ({
  duration = 280,
  easingType = 'cubic-bezier',
  easingAmount = '0.4, 0, 0.2, 1',
} = {}) => {
  return `;
    transition: box-shadow ${duration}ms ${easingType}(${easingAmount});
